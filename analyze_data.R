...
p_sub <- p_sub + geom_bar(aes(fill = type), stat="identity")
print(p_sub)
...
p_most <- p_most + geom_bar(aes(fill = type), stat="identity")
print(p_most)
...
p_cre <- p_cre + geom_bar(aes(fill = type), stat="identity")
print(p_cre)
...
# --------------------------------------------------------------------
to_sub = c('Loyer', 'Ammeublement', 'Soin_Hygiene', 'Sport', 'Voyages', 'Vetements')
...
p_dist_deb <- p_dist_deb + geom_bar(aes(fill = type), stat="identity") 
...
p_Vetements <- ggplot(data_keeped, aes(date_ope, total)) + geom_bar(aes(fill = type), stat="identity") 
#p_Vetements <- p_Vetements + ggtitle("Evolution de ma dépense au cours du temps")

# Sorties
data_keeped = filter(df_sum_deb_full, type == 'Sorties')
p_Sorties <- ggplot(data_keeped, aes(date_ope, total)) + geom_bar(aes(fill = type), stat="identity") 
p_Sorties <- p_Sorties + ggtitle("Obj: 115")

# Nourriture
data_keeped = filter(df_sum_deb_full, type == 'Nourriture')
p_Nourriture <- ggplot(data_keeped, aes(date_ope, total)) + geom_bar(aes(fill = type), stat="identity") 
p_Nourriture <- p_Nourriture + ggtitle("Obj: 200")

# Loisirs
data_keeped = filter(df_sum_deb_full, type == 'Loisirs')
p_Loisirs <- ggplot(data_keeped, aes(date_ope, total)) + geom_bar(aes(fill = type), stat="identity") 
#p_Loisirs <- p_Loisirs + ggtitle("Evolution de ma dépense au cours du temps")

# Charges
data_keeped = filter(df_sum_deb_full, type == 'Charges')
p_Charges <- ggplot(data_keeped, aes(date_ope, total)) + geom_bar(aes(fill = type), stat="identity") 
p_Charges <- p_Charges + ggtitle("Obj: 115")

# Autre
data_keeped = filter(df_sum_deb_full, type == 'Autre')
p_Autre <- ggplot(data_keeped, aes(date_ope, total)) + geom_bar(aes(fill = type), stat="identity") 
